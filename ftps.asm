...
print_data_status:
    ...
    mov   rax , 13
    mov   rbx , dx shl 32 + 17*6
    mov   rcx , (dy-1) shl 32 + 12
    mov   rdx , 0xffffff   
    int   0x60
...
sendDir:
  ...
  sd001:
    ...
    inc     dword [readdir]     ; Prepare for next entry read
    
    ; Do we have last entry ?      

    cmp     eax, 0
    jne     sd_exit

    inc     r15
    mov     [direntry+8+19-2],word '  '
    test    r15 , 11b
    jnz     nostrend
    mov     [direntry+8+19-2],word 13+10*256
  nostrend:
    ...
    cmp     [timeoutconnection],dword 60*100
    ja      sd_exit
...
sendFile:
  ...
  sf003:
    ...
    cmp     [timeoutconnection],dword 2*60*100
    ja      sf_exit
...
data_empty:         db  0                     
data_receiving:     db  'Receiving file..',0
data_sending:       db  'Sending file..',0
data_sending_list:  db  'Sending list..',0
...
loginStr0:          db  '220- ',13,10
                    db  '      Menuet FTP server v0.3',13,10
